...
public class SmartFontRenderer {
...
//    }
    ...
    public enum TextAlignment {
        LEFT_RIGHT((x, s) -> x), MIDDLE((x, s) -> x + (Minecraft.getInstance().font.width(s) / 2)), RIGHT_LEFT(((x, s) -> x - Minecraft.getInstance().font.width(s)));
        
        public final BiFunction<Integer, String, Integer> conversion;
        TextAlignment(BiFunction<Integer, String, Integer> conversion) {
            this.conversion = conversion;
        }
    }

    public enum TextShadow {
        NONE(false), NORMAL(true), OUTLINE(true);

        public final boolean shadow;

        TextShadow(boolean shadow) {
            this.shadow = shadow;
        }
    }
